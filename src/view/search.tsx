...
import VoteComponent from '@/view/index/components/vote'
...
export default function SearchContainer({
  ...
  const loadSearch = useCallback(
    async (search?: string, pageParams?: number) => {
      console.log('[search][SearchContainer] loadSearch called:', {
        search,
        pageParams,
      })
      if (!search) return
      ...
      if (search !== lastSearchValue.current) {
        ...
        if (currentSearchCount.current >= 10) {
          console.log(
            '[search][SearchContainer] Search count exceeded for current session'
          )
          return
        }
      ...
      try {
        ...
        if (!Array.isArray(result)) {
          console.error(
            '[search][SearchContainer] API result is not an array:',
            result
          )
          return
        }
        ...
        const requestPairs: Array<Promise<PairsResponse>> = parsePairs
          .filter(
            (item: any) =>
              item.address &&
              (!item.pair ||
                isTimeExceededByOneMinute(item.pair?.timestamp || 0))
          )
          .map((item: any) => getPairByTokens(item.address))

        console.log(
          '[search][SearchContainer] requesting pairs:',
          requestPairs.length
        )
        ...
        if (requestPairs.length > 0) {
          const pairs = (await Promise.all(requestPairs)).reduce(
            (
              acc: { [key: string]: Pair | undefined },
              pairs: PairsResponse
            ) => {
              if (!pairs || !pairs.address) {
                console.log(
                  '[search][SearchContainer] Invalid pairs response:',
                  pairs
                )
                return acc
              }
              const tokenAddress = pairs.address
              const maxPair = pairs?.pairs?.reduce(
          ...
          if (updatePairs.length) {
            ...
            await updatePairsAPI(
              updatePairs.map(item => ({ ...item, timestamp })),
              '123'
            )
          }
        }
        ...
        const newSearchData =
          pageParams === 1 ? searchs : [...searchData, ...searchs]

        console.log(
          '[search][SearchContainer] final search data:',
          newSearchData
        )
  ...
  return (
    <div className="search-bar z-50 w-full flex-1">
      <Container className="">
        <Grid columns={1} gap={20}>
          <Grid.Item className="grid grid-cols-[1fr_auto] items-center gap-4">
            <Search
              ...
              onChange={async (search: string) => {
                console.log(
                  '[search][SearchContainer] Search onChange:',
                  search
                )
                setSearchValue(search)
                setPage(1)
